...
*1. Log Loss (логарифмическая потеря)*
Log Loss измеряет, насколько хорошо модель предсказывает вероятности классов. Чем ниже значение Log Loss, тем точнее модель. Эта метрика особенно важна для задач, где требуется предсказать вероятность одобрения заявки.Формула для расчета Log Loss выглядит так:
...
*2. F1-Score*
F1-Score — это гармоническое среднее между точностью (precision) и полнотой (recall). Эта метрика особенно полезна для задачи прогнозирования кредита, где важно минимизировать как ложные отказы (FN), так и ложные одобрения (FP).
...
*3. Accuracy*
Accuracy измеряет долю правильных предсказаний среди всех предсказаний. Это простой и популярный метод оценки классификационных моделей, который показывает, насколько часто модель правильно предсказывает статус заявки.
...
*4. ROC-AUC*
ROC-AUC оценивает, насколько хорошо модель различает положительные и отрицательные классы. Это полезно в задачах, где требуется взвешенное решение для минимизации как ложноположительных, так и ложноотрицательных результатов.
